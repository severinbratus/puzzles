...
def prettyprint(mapa : dict | set, path : list[Point], filename : str, finish_points : set[Point]):
...
def char_repr(point, mapa, path, finish_points):
    if point not in mapa: return ' '
    if point in finish_points: return 'F'
    if point in path: return '_'
    return '.'
...
if __name__ == '__main__':
    ...
    prettyprint(tissue, [], 'tissue.txt', finish_points)
    prettyprint(tissue, path, 'tissue-pathed.txt', finish_points)
